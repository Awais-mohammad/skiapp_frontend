...
export class CoursePaymentDetailsPage extends BasicUserIdPage implements OnInit {
  ...
  invoice:CourseRegistrationInvoice = null;
  ...
  ngOnInit() {
    ...
    if(!this.providerId || !this.invoice){
  ...
  ionViewDidEnter() {
    ...
    if(this.invoice && this.invoice.amount>0){
      if(this.invoice.statusId===this.appConstants.PAYMENT_UNPAID_ID ||
        this.invoice.statusId===this.appConstants.PAYMENT_TOPAY_OFFLINE_ID ||
        this.invoice.statusId===this.appConstants.PAYMENT_TOPAY_ONLINE_ID) {
  ...
  ionViewCanLeave(){
    ...
    if(!this.invoice ||
        this.invoice.statusId!==this.appConstants.PAYMENT_UNPAID_ID ||
  ...
  l_checkingCallback(context_this:any){
    ...
    context_this.coursePaymentService.s_getCourseInvoiceById(context_this.appSession.l_getUserId(), context_this.invoice.id,
      (invoice:CourseRegistrationInvoice) => {
        ...
        if(!invoice || invoice.statusId === context_this.appConstants.PAYMENT_CANCELLED_ID || invoice.statusId === context_this.appConstants.PAYMENT_FAILED_ID ||
          ...
          let navigationExtras: NavigationExtras = {
            state: {
              registrationId:context_this.invoice.registrationId,
    ...
    context_this.coursePaymentService.s_getCourseInvoiceById(context_this.appSession.l_getUserId(), context_this.invoice.id,
      (invoice:CourseRegistrationInvoice) => {
        if(invoice && invoice.statusId === context_this.appConstants.PAYMENT_FULLY_ID || invoice.statusId === context_this.appConstants.PAYMENT_TOPAY_OFFLINE_ID ||
          ...
          // paymentAction.invoiceId = context_this.invoice.id;
          ...
          //   context_this.invoice.id, (result:boolean) => {
  ...
  async onViewAmountDetails(){
    ...
    if(this.invoice){
      const modal = await this.modalController.create({
        ...
        componentProps: {invCal:this.invoice.invoiceCalculation},
      });
      await modal.present();
  ...
  onClose(){
    if(this.invoice && this.paymentProcessUtil.isCourseRequirePayment(this.invoice.statusId)){
  ...
  l_cancalPayment(){
    console.log("Cancel payment from client.");
    this.coursePaymentService.s_cancelPaymentByClient(this.providerId, this.appSession.l_getUserId(), this.invoice.id, (result: boolean) => {
      if (result) {
  ...
  async l_initialPayment(){
    ...
    this.paymentService.s_generateClientTokent(l_this.appSession.l_getUserId(), l_this.providerId, (generalResponse:GeneralResponse) => {
      ...
      if(clientTokent){
        ...
        let payAmount = l_this.invoice.amount;
        ...
        if(payAmount<=0){
          ...
          button.addEventListener('click', function () {
            ...
            paymentAction.courseInvoiceId = l_this.invoice.id;
  ...
  async l_processPaymentOnServer(instance, amount, l_this){
    ...
    instance.requestPaymentMethod(function (requestPaymentMethodErr, payload) {
      ...
      if(payload && payload.nonce){
        ...
        let paymentAction = new PaymentAction();
        paymentAction.amount = l_this.invoice.amount;
        paymentAction.providerId = l_this.providerId;
        paymentAction.userId = l_this.appSession.l_getUserId();
  ...
  l_sendPayment(l_this, paymentAction:PaymentAction){
    ...
    l_this.registrationId = l_this.invoice.registrationId;
  ...
  async l_verifyAndPrepay(email:string, code:string){
    ...
    let verifyRequest = new VerifyPreApprovedPaymentRequest(this.providerId, email, code, this.invoice.id);
    this.coursePaymentService.s_verifyPreApprovedPayment(verifyRequest, (genResponse:GeneralResponse) => {
      if(genResponse && genResponse.resultObject){
        ...
        let paymentAction = new PaymentAction();
        paymentAction.amount = this.invoice.amount;
        paymentAction.providerId = this.providerId;
        paymentAction.userId = this.appSession.l_getUserId();
        paymentAction.courseInvoiceId = this.invoice.id;
