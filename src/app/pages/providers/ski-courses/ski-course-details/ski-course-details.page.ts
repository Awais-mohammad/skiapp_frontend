...
export class SkiCourseDetailsPage extends BasicUserIdPage implements OnInit, OnDestroy {
  ...
  async showInstructorsSelection(instructors:InstructorWithDetails[]){
    ...
    const alert = await this.alertCtrl.create({
      header: this.translateUtil.translateKey("Choose instructor"),
      inputs: inputs,
      buttons: [
        {
  ...
  getRegistrationLine(registration:PackageCourseRegistration):string{
    if(!registration){
      return null;
    }
  ...
  async l_guestCheckOut(){
    ...
    let guestRegistration:PackageCourseRegistration = data;
  ...
  private doRegisterCourse(registration:PackageCourseRegistration){
    let navigationExtras: NavigationExtras = {
      state: {
  ...
  l_prepareRegistration():PackageCourseRegistration{
    // create registration;
    let startTime = null;
    let endTime = null;
    ...
    let registrationRequest = new PackageCourseRegistration();
  ...
  async onAddRegistration(){
    ...
    await modal.present();
    const { data } = await modal.onDidDismiss();
    let selectedRegistration:PackageCourseRegistration = data;
