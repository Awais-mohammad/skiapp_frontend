...
export class SkiCoursesRegistrationsPage extends BasicUserIdPage implements OnInit {
  ...
  courseRegistrations:PackageCourseRegistration[] = null;
  callback:any = null;
  ...
  updatePageContent(){
    ...
    if(this.appType===this.seg_course){
      ...
      if(this.fromCommand===this.appConstants.PAGE_FOR_PROVIDER && this.providerId>0){
        this.providersService.s_getCourseRegistrationsForProviderId(this.providerId, null, false, this.futureOnly, (registrations:PackageCourseRegistration[]) => {
        ...
        this.providersService.s_getRegistrationsForInstructorId(this.instructorId, this.futureOnly, (registrations:PackageCourseRegistration[]) => {
          this.sortRegistrations(registrations);
          this.courseRegistrations = registrations;
        });
      }else if(this.fromCommand===this.appConstants.PAGE_FOR_MEMBER){
        this.providersService.s_getRegistrationsForUserId(this.providerId, this.userId, this.futureOnly, (registrations:PackageCourseRegistration[]) => {
          this.sortRegistrations(registrations);
          this.courseRegistrations = registrations;
        });
      }else{
  ...
  sortRegistrations(registrations:PackageCourseRegistration[]){
  ...
  onViewCourseRegistrationDetails(registration:PackageCourseRegistration){
  ...
  getFirstStartTime(registration:PackageCourseRegistration){
  ...
  searchCourseRegistrationsByUser(userInfo:UserInfo){
    ...
    this.providersService.s_getRegistrationsForUserId(this.providerId, userInfo.id, false, (regs:PackageCourseRegistration[]) => {
  ...
  searchCourseRegistrationsForEmail(email:string){
    ...
    this.providersService.searchCourseRegistrationsForEmail(this.providerId, email, true, (regs:PackageCourseRegistration[]) => {
      this.courseRegistrations = regs;
    });
  }
