...
export class PaymentProcessUtil {
  ...
  checkAnyPayCourse(registration:PackageCourseRegistration, navCtrl:NavController, payOfflineOnly:boolean){
    this.l_checkProviderAndRegister(registration, navCtrl, payOfflineOnly);
  }

  l_checkProviderAndRegister(registration:PackageCourseRegistration, navCtrl:NavController, payOfflineOnly:boolean){
  ...
  l_goConfirmAndPay(userId:number, registration:PackageCourseRegistration, navCtrl:NavController, payOfflineOnly:boolean){
    if(!registration){
  ...
  processRegistrationResponse(payOfflineOnly:boolean, registration:PackageCourseRegistration, response:GeneralResponse){
    if(!response){
      return null;
    }
    ...
    if(invoice){
      if(invoice.statusId!==this.appConstants.PAYMENT_FULLY_ID && invoice.statusId!==this.appConstants.PAYMENT_WAIVED_ID &&
        ...
        this.toastUtil.showToast("Unknown payment status. Please choose another time and try again.");
      }
    }else{
