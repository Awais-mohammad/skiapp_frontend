...
export class CoursePaymentService {
  ...
  /******************* testing service ****************/
  simpleCheckOut(providerId:number, userId:number, paymentAction:PaymentAction, callback?){
    if(!paymentAction){
      return;
    }
    console.log("simpleCheckOut.");
    this.aclService.s_getURL( (serviceUrl:string) => {
      this.httpUtil.s_call(
        "simpleCheckOut",  // objName
        serviceUrl + "simpleCheckOut/" + providerId + "/" + userId, //urlStr,
        "post", //method,
        paymentAction, //requestObj,
        null, //keyStr,
        null, //groupKeyStr,
